...
def disparity_mapping(left_image, right_image, rgb=rgb_value):
    ...
    matcher = cv2.StereoSGBM_create(numDisparities=num_disparities, #whats this ?
                                    minDisparity=0,
...
def feature_extractor(image, detector=detector_name, mask=None):
    ...
    all_keypoints = []
    all_descriptors = None  # We'll stack descriptors as a numpy array

    # Process each bucket.
    for row in range(num_rows):
        for col in range(num_cols):
            # Define the bucket region.
            x_start = col * bucket_width
            y_start = row * bucket_height
            # Ensure we cover the whole image on the rightmost and bottommost buckets.
            x_end = x_start + bucket_width if col < num_cols - 1 else w
            y_end = y_start + bucket_height if row < num_rows - 1 else h

            # Extract the bucket region.
            bucket_img = image[y_start:y_end, x_start:x_end]

            # Compute ORB keypoints and descriptors for the bucket.
            keypoints, descriptors = orb.detectAndCompute(bucket_img, None)
    ...
    # all_keypoints and all_descriptors now contain features from across all buckets.
